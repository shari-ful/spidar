...
from spidar.builder import (
    builder_registry,
    TreeBuilder,
    ParserRejectedMarkup,
)
from spidar.element import (
    Comment,
    SoupStrainer,
...
class TestConstructor(SoupTest):
    ...
    def test_custom_builder_class(self):
        ...
        with warnings.catch_warnings(record=True):
            soup = Spidar('', builder=Mock, **kwargs)
        ...
        # to the Spidar constructor are ignored.
        builder = Mock(**kwargs)
        ...
        msg = str(w[0].message)
        assert msg.startswith("Keyword arguments to the Spidar constructor will be ignored.")
    ...
    def test_cdata_list_attributes(self):
        ...
        # by passing in a keyword argument to the Spidar constructor.
...
class TestOutput(SoupTest):
    ...
    def test_decode_xml_declaration_with_missing_or_python_internal_eventual_encoding(self, eventual_encoding):
        ...
        soup = Spidar("<tag></tag>", "html.parser")
        soup.is_xml = True
        assert (f'<?xml version="1.0"?>\n<tag></tag>'
                == soup.decode(eventual_encoding=eventual_encoding))

    def test(self):
        # Spidar subclasses Tag and extends the decode() method.
...
class TestWarnings(SoupTest):
    # Note that some of the tests in this class create Spidar
    ...
    def _assert_no_parser_specified(self, w):
        ...
        assert message.startswith(Spidar.NO_PARSER_SPECIFIED_WARNING[:60])
    ...
    def test_resembles_filename_warning(self, extension):
        ...
        with warnings.catch_warnings(record=True) as w:
            soup = Spidar("markup" + extension, "html.parser")
    ...
    def test_url_warning_with_bytes_url(self):
        url = b"http://www.spidarbytes.com/"
        with warnings.catch_warnings(record=True) as warning_list:
            soup = Spidar(url, "html.parser")
    ...
    def test_url_warning_with_unicode_url(self):
        url = "http://www.spidarunicode.com/"
        with warnings.catch_warnings(record=True) as warning_list:
    ...
    def test_url_warning_with_bytes_and_space(self):
        ...
        # is issued.
        with warnings.catch_warnings(record=True) as warning_list:
            soup = self.soup(b"http://www.spidarbytes.com/ is great")
        assert not any("looks more like a URL" in str(w.message) 
                       for w in warning_list)

    def test_url_warning_with_unicode_and_space(self):
        with warnings.catch_warnings(record=True) as warning_list:
            soup = self.soup("http://www.spidarunicode.com/ is great")
        assert not any("looks more like a URL" in str(w.message) 
                       for w in warning_list)
...
class TestNewTag(SoupTest):
    """Test the Spidar.new_tag() method."""
...
class TestNewString(SoupTest):
    """Test the Spidar.new_string() method."""
...
class TestPickle(SoupTest):
   # Test our ability to pickle the Spidar object itself.
...
class TestEncodingConversion(SoupTest):
    # Test Spidar's ability to decode and encode from various
    # encodings.
